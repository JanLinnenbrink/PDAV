...
samples_pred <- terra::spatSample(prediction, 1000, as.points = TRUE, na.rm = TRUE, method = "regular") |>
	st_as_sf() |>
	rename("prediction" = lyr1)
samples_pred <- terra::extract(r$outcome, samples_pred, bind = TRUE)

rmse <- sqrt(mean((samples_pred$prediction - samples_pred$outcome)^2))
rmse
```
