...
#' @references
#' \itemize{
#' \item Linnenbrink, J., Milà, C., Ludwig, M., and Meyer, H.: kNNDM: k-fold Nearest Neighbour Distance Matching Cross-Validation for map accuracy estimation, EGUsphere, https://doi.org/10.5194/egusphere-2023-1308, 2023.
  ...
  if(testks$p.value >= 0.05){
    ...
    for(nk in clustgrid$nk){
      ...
      centr_tpoints <- sapply(tabclust$clust_nk, function(x){
        ...
        return(stats::predict(pcacoords, centrpca))
      })
      ...
      if (!inherits(clust_nk,"error")){
        ...
        if(is.null(catVars)) {
          ...
          centr_tpoints <- sapply(tabclust$clust_nk, function(x){
            ...
            return(stats::predict(pcacoords, centrpca_num, centrpca_cat)[,1])
